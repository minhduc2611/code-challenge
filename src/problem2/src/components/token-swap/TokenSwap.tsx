...
const styles = {
  ...
  input:
    "*:*:w-full *:*:outline-none text-gray-600 text-4xl border-none mt-1 p-2 border w-[70%] rounded-md outline-none focus:border-blue-500 disabled:*:*:bg-white disabled:*:*:cursor-not-allowed disabled:*:*:opacity-50",
        ...
        <form className={styles.form}>
          ...
          <FlipMove>
            {swapArray.map((el, index) => {
              return (
                <div key={swaps[el].token + index} className={`border z-11 rounded-xl p-2 ${index == 0 && "mb-12"}`}>
                  <label className={styles.label}>{index === 0 ? "From" : "To (estimated)"}</label>
                  <div className={styles.inputWrapper}>
                    ...
                    />
                    <TokenSwapSelector value={swaps[el].token} onChange={(val) => setToken(el, val)}/>
                  </div>
                  <div className={styles.balance}>
                    <span>
                      ${computedPrice}{" "}
                      {index !== 0 && computedPrice !== 0 && swaps[el].token !== DEFAULT_TOKEN && (
                          <span title="slippage" className="text-[#e74040] cursor-default ml-2">0,05%</span>
                        )}
                    </span>
                    <span
                      className={cn(
                    ...
                    >
                      Balance:{" "}
                      <span className="cursor-pointer" onClick={() => setAmount(el, balanceMap[swaps[el].token].balance)}>
                        {swaps[el].token !== DEFAULT_TOKEN ? balanceMap[swaps[el].token].balance : 0}
                      </span>
                    </span>
                  </div>
                </div>
          ...
          <button
            ...
            type="button"
            disabled={isDisabled}
            className={cn(styles.swapButton, disabled && "text-[#c5f0ea] cursor-not-allowed")}
          >
            Swap {!isDisabled && "✨"}
          </button>
        </form>
